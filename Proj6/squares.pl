...
insideBigSquare(0, _, [], []).
insideBigSquare(N, Big, [Ss|Sides], [Ps|Position]):-
	Ps #> Big - Ss,
	Ns is N - 1,
	insideBigSquare(Ns, Big, Sides, Position).

nonoverlapping(0, [], [], []).
nonoverlapping(N, [S|Sides], [R|RowVars], [C|ColVars]):-
	check(S, R, C, Sides, RowVars, ColVars),
	Ns is N - 1,
	nonoverlapping(Ns, Sides, RowVars, ColVars).

check(_, _, _, [], [], []):-true.
check(S, R, C, [Ss|Sides], [Rs|RowVars], [Cs|ColVars]):-
	R > Rs + Ss or Rs >= R + S or C >= Cs + Ss or C2 >= C + S,
	check(S, R, C, Sides, RowVars, ColVars).
