...
lowerBoundOfRemainingCities([X], LBound):- minim(X, 22, LBound).
lowerBoundOfRemainingCities([X|Cities], LBound) :- minim(X, 22, LBound), lowerBoundOfRemainingCities([Cities], LBounds), LBound < LBounds.
lowerBoundOfRemainingCities([X|Cities], LBound) :- minim(X, 22, Min), lowerBoundOfRemainingCities([Cities], LBound), Min >= LBound.

minim(N, 0, Min) :- distance(N, X, Min), Min /= 0.
minim(N, 0, 100000) :- distance(N, X, Km), Km == 0.

minim(N, X, Min) :- minim(N, X-1, Min1), distance(N, X, Km), Km /= 0, Km > Min1, Min = Min1.
minim(N, X, Min) :- minim(N, X-1, Min1), distance(N, X, Km), Km /= 0, Km <= Min1, Min = Km.
%3
